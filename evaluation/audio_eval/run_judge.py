...
Usage:
    ...
    python -m audio_eval.run_judge results/audio_eval/my_run_final.json

    # Use a different judge model
    python -m audio_eval.run_judge results/audio_eval/my_run_final.json --judge-model gpt-4o
    ...
    python -m audio_eval.run_judge results/audio_eval/my_run_final.json --batch-size 20

    # Only re-score entries that don't already have an llm score
    python -m audio_eval.run_judge results/audio_eval/my_run_final.json --fill-missing

    # Write output to a new file instead of overwriting
    python -m audio_eval.run_judge results/audio_eval/my_run_final.json \\
        -o results/audio_eval/my_run_judged.json
"""
...
from . import config
from .metrics import aggregate_results, batch_llm_judge, compute_score_distribution
